...
Copy `.env.example`, fill in `SUPABASE_DATABASE_URL`, `SUPABASE_URL`, and `SUPABASE_SECRET_KEY` (required for seed/reset). For local HTTP, set `COOKIE_SECURE=false`.
...
- **DB**: Drizzle ORM + pg Pool. Client from `src/shared/models/index.ts` (exports `db` and `pool`).
...
- **Supabase clients** (`src/shared/lib/supabase.ts`): `supabaseAdmin` (service role) and `supabaseAuth` (anon key).
- **Auth middleware** (`src/shared/middlewares/auth.ts`): tries local JWT first. Expired JWT throws immediately (no fallback). Other JWT failures fall back to Supabase `getUser(token)`. Sets `req.employee`. `requireRole("barista" | "manager" | "owner")` gates endpoints.
...
- **File uploads**: `multer` (memory storage). Images only (JPEG, PNG, GIF, WebP, SVG, BMP, TIFF), max 5MB. Files go to Supabase Storage.
- **Soft deletes**: Most tables use `deletedAt` timestamp — queries should filter `isNull(deletedAt)`.

## Seed and reset

Both require `SUPABASE_URL` + `SUPABASE_SECRET_KEY`.

```
npm run db:reset && npm run db:seed
```

`db:seed` is idempotent (`onConflictDoNothing`), auto-creates Supabase Auth users. Override employees with `SEED_EMPLOYEES` env var (JSON array). `db:reset` truncates `RESTART IDENTITY CASCADE` and deletes all Supabase Auth users — destructive, never in production.
...
- `src/shared/utils/swagger.ts` imports logger without `.ts` extension — existing inconsistency.
- `src/shared/middlewares/validate.ts` imports `AppError` without `.ts` extension — another inconsistency.
...
- Categories use `router.use(authenticate)` at router level; all other domains apply auth per-route.
- Auth routes (`/api/auth/*`) are public except `POST /auth/signup` (owner-only).
- Employee routes are **public** at the router level (no `authenticate` in the mount) — each route applies its own auth.
- `.prettierignore` excludes `AGENTS.md`, `.agents/`, `src/shared/models/migrations/`.
